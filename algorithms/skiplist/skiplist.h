 ...
 * https://github.com/hit9/C-Snip/blob/master/src/skiplist.h
...
#define skiplist_each(sl, node)                            \
    for ((node) = (sl)->head->forwards[0]; (node) != NULL; \
...
int skiplist_push(struct skiplist *skiplist, unsigned long score,
                  void *data); /* O(logN) */
void *skiplist_get(struct skiplist *skiplist,
                   unsigned long score); /* O(logN) */
void *skiplist_pop(struct skiplist *skiplist,
                   unsigned long score);                         /* O(logN) */
