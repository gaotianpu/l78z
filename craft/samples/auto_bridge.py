...
def roundVec3(vec3):
    ...
    # return mcpi.vec3.Vec3(int(vec3.x), int(vec3.y), int(vec3.z))

if __name__ == "__main__":

    time.sleep(2)  #为啥这个要先sleep 2 ms?
    ...
    while (True):
        ...
        #   ((movementX < -0.2) or (movementX > 0.2) or (movementZ < -0.2) or (movementZ > 0.2))
        #  玩家在移动、跑步、潜行模式下，一步分别能走几个方块？ 
        if  abs(movementX)>0.2 or abs(movementZ)>0.2:
            ...
            while ((int(playerPos.x) == int(nextPlayerPos.x)) and (int(playerPos.z) == int(nextPlayerPos.z))):
                ...
                nextPlayerPos = roundVec3(nextPlayerPos.x - movementX, nextPlayerPos.y, nextPlayerPos.z - movementZ) 
