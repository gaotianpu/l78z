...
; (function () {
    function main() {
        ...
        class Line {
            ...
            //左边界点
            get left_point() {
                return new Point(0, Math.round(this.bias));
            }
        ...
        //计算两点之间的距离 
        function calc_distance(item, exact = false) {
            // var ret = Math.sqrt(Math.pow(item.x - item.x1, 2) + Math.pow(item.y - item.y1, 2));
            var ret = Math.hypot(item.x - item.x1, item.y - item.y1);
            if (exact) {
                return ret;
            }
