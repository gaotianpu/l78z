...
from rnn_train_point import PointWiseModel,StockDataset
...
def run_predict(model_dir="model"):
    ...
    model = torch.load('%s/rnn_point_5.pth'%(model_dir))
    model.eval() #预测模式
    with torch.no_grad():
        li = []
        ...
        with open("predict/rnn_predict_scores_%d.txt"%(trade_date),'w') as f:
            f.write(content)
        with open("predict/rnn_predict_scores.txt",'w') as f:
            f.write(content)
