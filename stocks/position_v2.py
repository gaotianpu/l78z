...
def process():
    df = pd.read_csv("data/predict_v2/predict_merged.txt.20231024",sep=";",header=0) 
    li_all = []
    for index, row in df.iterrows():
        ...
        stocks_file = "data/boost_stocks_train/boost_%s.txt" % (stock_no)
        
        if not os.path.exists(stocks_file):
            print(stocks_file,"file is not exist")
            continue 
        
        # sql = "select * from stock_for_boost_v2 where stock_no='%s'" % (stock_no)
        # df_stocks = pd.read_sql(sql, conn)     
        df_stocks = pd.read_csv(stocks_file,sep=";",header=0)
    ...
    df.to_csv("data/position_v2.txt",sep=";",index=False)
