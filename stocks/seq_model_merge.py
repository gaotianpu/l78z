...
def model_intersection():
    ...
    # pair_15,list_dates
    df_predict_v1 = df_predict_v1.sort_values(by=["point2pair_dates_top5","list_dates_top5"],ascending=False)
...
def merge_one(trade_date,df_predict,df1,df2,df3,version='v2'):
    df_predict = df_predict.merge(df1,on="stock_no",how='left')
    df_predict = df_predict.merge(df2,on="stock_no",how='left')
    df_predict = df_predict.merge(df3,on="stock_no",how='left')
    
    df_predict['low_rate'] = c_round((df_predict['LOW_1']-df_predict['CLOSE_price'])/df_predict['CLOSE_price'])
    df_predict['high2_rate'] = c_round( (df_predict['high_2'] - df_predict['CLOSE_price'])/df_predict['CLOSE_price'] )
    df_predict['high3_rate'] = c_round( (df_predict['high_3'] - df_predict['CLOSE_price'])/df_predict['CLOSE_price'] )
    
    for i in  range(5):
        df_predict[f'point_low1_buy_{i}'] = df_predict.apply(lambda x: 0 if x['low_rate']>=x[f'point_low1_{i}'] else 1, axis=1)
        df_predict[f'high2_rate_{i}'] = df_predict.apply(lambda x: 0 if x['low_rate']>=x[f'point_low1_{i}'] else (x['high2_rate'] - x[f'point_low1_{i}']), axis=1)
        df_predict[f'high3_rate_{i}'] = df_predict.apply(lambda x: 0 if x['low_rate']>=x[f'point_low1_{i}'] else (x['high3_rate'] - x[f'point_low1_{i}']), axis=1)
...
def merge_predict_true(start_date=20231017):
    ...
    for idx,trade_date in enumerate(trade_dates):
        ...
        # 第一天，昨收，开盘价，最低价
        true_date = trade_dates[idx+0]
        # (CLOSE_price-change_amount) as last_close,
        sql = f"select stock_no,OPEN_price as OPEN_1,LOW_price as LOW_1 from stock_raw_daily_2 where trade_date={true_date}"
        df1 = pd.read_sql(sql, conn)
        
        # 第二天，最高价
        true_date = trade_dates[idx+1]
        sql = f"select stock_no,HIGH_price as high_2 from stock_raw_daily_2 where trade_date={true_date}"
        df2 = pd.read_sql(sql, conn)
        
        # 第三天，最高价
        true_date = trade_dates[idx+2]
        sql = f"select stock_no,HIGH_price as high_3 from stock_raw_daily_2 where trade_date={true_date}"
        ...
        # merge_one(trade_date,df_predict_v1,df1,df2,df3,'v1')
        
        df_predict_v2 = pd.read_csv(f"data/predict_v2/predict_merged.txt.{trade_date}",sep=";",header=0,dtype={'stock_no': str})
        statics_li = statics_li + merge_one(trade_date,df_predict_v2,df1,df2,df3,'v2')
        # if idx>1:
        #     break
    ...
    print(len(statics_li),len(statics_li[0]),len(columns))
