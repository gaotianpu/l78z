...
FIELDS_PRICE="OPEN_price;CLOSE_price;LOW_price;HIGH_price;last_close".split(";")
...
class PreProcessor:
    ...
    def process_row(self, df, idx,current_date):
        ...
        if idx>0: #train
            ...
            high2_rate = compute_rate(df.loc[idx-2]['HIGH_price'],hold_base)
            low1_high2 = high2_rate-next_low_rate
        ...
        max_price = df_past['HIGH_price'].max()
        min_price = df_past['LOW_price'].min()
        min_max_rate={}
        for rate in FIELDS:
            min_max_rate[rate]=[df_past[rate].min(),df_past[rate].max()]
